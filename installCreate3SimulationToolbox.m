function installCreate3SimulationToolbox(replaceExisting)
% INSTALLCREATE3SIMULATIONTOOLBOX installs Create3 Simulation Toolbox for 
% MATLAB
%
%   INSTALLCREATE3SIMULATIONTOOLBOX installs Create3 Simulation Toolbox for 
%   MATLAB into the following locations:
%                             Source: Destination
%  Create3SimulationToolboxFunctions: matlabroot\toolbox\create3simulation
%
%   INSTALLCREATE3SIMULATIONTOOLBOX(true) installs Create3 Simulation  
%   Toolbox regardless of whether a copy of the Create3 Simulation toolbox
%   exists in the MATLAB root.
%
%   INSTALLCREATE3SIMULATIONTOOLBOX(false) installs Create3 Simulation 
%   Toolbox only if no copy of the Create3 Simulation Simulation toolbox 
%   exists in the MATLAB root.
%
...
if isToolbox == 7
    ...
    if isempty(replaceExisting)
        choice = questdlg(sprintf(...
            ['MATLAB Root already contains the Create3 Simulation Toolbox.\n',...
            'Would you like to replace the existing toolbox?']),...
            'Replace Existing Create3 Simulation Toolbox','Yes','No','Cancel','Yes');
    ...
    switch choice
        case 'Yes'
            ...
            if isRemoved
                fprintf('Previous version of Create3 Simulation Toolbox for MATLAB removed successfully.\n');
            else
                fprintf('Failed to remove old Create3 Simulation Toolbox for MATLAB folder:\n\t"%s"\n',toolboxRoot);
                fprintf(adminSolution);
                error(msgID,msg);
            end
        case 'No'
            fprintf('Create3 Simulation Toolbox for MATLAB currently exists, installation cancelled.\n');
            return
        case 'Cancel'
...
if isDir
    fprintf('Create3 Simulation Toolbox folder created successfully:\n\t"%s"\n',toolboxRoot);
else
    fprintf('Failed to create Create3 Simulation Toolbox folder:\n\t"%s"\n',toolboxRoot);
    fprintf(adminSolution);
    error(msgID,msg);
end

%% Migrate toolbox folder contents
toolboxContent = 'Create3SimulationToolboxFunctions';
if ~isdir(toolboxContent)
    error(sprintf(...
        ['Change your working directory to the location of "installCreate3SimulationToolbox.m".\n',...
         '\n',...
         'If this problem persists:\n',...
         '\t(1) Unzip your original download of "Create3SimulationToolbox" into a new directory\n',...
         '\t(2) Open a new instance of MATLAB "as administrator"\n',...
         '\t\t(a) Locate MATLAB shortcut\n',...
         '\t\t(b) Right click\n',...
         '\t\t(c) Select "Run as administrator"\n',...
         '\t(3) Change your "working directory" to the location of "installCreate3SimulationToolbox.m"\n',...
         '\t(4) Enter "installCreate3SimulationToolbox" (without quotes) into the command window\n',...
         '\t(5) Press Enter.']));
end
files = dir(toolboxContent);
wb = waitbar(0,'Copying Create3 Simulation Toolbox for MATLAB toolbox contents...');
n = numel(files);
fprintf('Copying Create3 Simulation Toolbox for MATLAB contents:\n');
for i = 1:n
